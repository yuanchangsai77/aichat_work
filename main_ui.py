...
# 去掉字符串前面的换行符，后面的len(parts) > 1判断若有换行符，返回parts[1]是换行符后面的字符串，若没有换行符，返回空字符串
...
class ui_surface:
    ...
    def __init__(self):
        ...
        self.sandbox = None#沙盒，用于存储对话信息
        self.query = ''#用户输入
        self.init_sandbox()#初始化沙盒

    # 初始化沙盒
    def init_sandbox(self):
        if self.valid_config:
            self.sandbox = Sandbox(self.base_config.world_name)#创建沙盒
            self.sandbox.init_global_agent(self.base_config)#初始化全局代理

    # 用户输入处理
    def user_msg_process(self, query, chat_history):#query是用户输入，chat_history是聊天历史
        self.query = query
        return gr.update(value="", interactive=False), \
            chat_history + [[self.base_config.user_name + '说: ' + query, None]]#返回用户输入和聊天历史 
    ...
    def get_response(self, chat_history):
        ...
        for chunk_ans in self.sandbox.chat(self.query):#遍历沙盒的聊天信息
            if chunk_ans is not None:#如果聊天信息不为空
                chat_history[-1][1] += chunk_ans#将聊天历史的最后一个元素的第二个元素设置为聊天信息
    ...
    def get_response_retry(self, chat_history):
        ...
        try:
            ...
            raise IndexError("没有历史提问。")#如果没有历史提问，抛出异常
        for chunk_ans in self.sandbox.chat(command_start + command_config['LIST']['retry']):#遍历沙盒的聊天信息
            if chunk_ans is not None:#如果聊天信息不为空
                chat_history[-1][1] += chunk_ans#将聊天历史的最后一个元素的第二个元素设置为聊天信息
                time.sleep(0.05)
                yield chat_history#返回聊天历史
    ...
    def print_debug_msg(debug_msg):
        debug_res = debug_msg_pool.get_msg()#获得调试pool信息
        cur_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")#获取当前时间
        debug_msg += ('---' + cur_time + '---' + '\n' + debug_res + '\n')#将调试信息添加到调试信息中
        debug_msg_pool.clear()#清空调试pool信息
        return debug_msg#返回调试信息

    # 重试消息处理
    def retry_msg_process(self, user_msg, chat_history):
        if self.query == '':#如果用户输入为空
            debug_msg_pool.append_msg("当前没有提问，无法重试。")#添加调试信息
            return user_msg, chat_history#返回用户输入和聊天历史
        try:
            chat_history[-1] = [self.base_config.user_name + '说: ' + self.query, None]#将聊天历史的最后一个元素的第二个元素设置为用户输入
        except IndexError:
            chat_history = [self.base_config.user_name + '说: ' + self.query, None]#将聊天历史的第二个元素设置为用户输入
        return gr.update(value="", interactive=False), chat_history#返回用户输入和聊天历史
    ...
    def start(self):
        with gr.Blocks(css=css) as demo:
            with gr.Tab("chat"):
                ...
                with gr.Column():
                    with gr.Row():
                        user_msg = gr.Textbox(label='Send a message',
                                              placeholder="按回车提交")#用户输入

                    with gr.Accordion(label='指令', open=False):#指令
                        with gr.Row():
                ...
                # 用户信息提交
                # then是异步执行，先执行fn=self.user_msg_process，再执行fn=self.get_response，再执行fn=self.print_debug_msg，最后gr.update(interactive=True)
            ...
            with gr.Tab("config"):
                with gr.Accordion(label='基本设置', open=True):# Accordion是可折叠的容器，作用是隐藏和显示内容
